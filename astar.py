...
class Cell():
    ...
    def draw(self):
        COST_TO_DRAW = ''
        # COST_TO_DRAW = self.g_cost # the number of steps it takes to get to that block
        # COST_TO_DRAW = self.h_cost # an estimation as to how far Paul is from the cake
        COST_TO_DRAW = self.f_cost # is g+h
        line_width = 2
...
# diagonals
# Since there's a set of diagonal wall, Paul must go diagonal to go through

# hops
# Diagonals won't suffice this time: Paul needs to hop.

# swamps
# Paul would rather go through the swamp than go the long way. He also goes diagonal 'cause he don't give a fuck (and to avoid the other swamp).