...
class Paul(Actor):
    ...
    def get_open_adj_coords(self, coords):
        ...
        #in_bounds = [self.world._is_in_grid(c) and self.world._is_occupied(c) and c not in self.closed_list for c in adj_coords]
        in_bounds = [self.world._is_in_grid(c) and not self.world._is_occupied(c) and c not in self.closed_list for c in adj_coords]
        #out_bounds = [self.world._is_in_grid(c) and self.world._is_occupied(c) and c not in self.closed_list for c in adj_coords]
        adj_coords = [c for (idx,c) in enumerate(adj_coords) if in_bounds[idx]]
        #print out_bounds[idx]
        #print (in_bounds, out_bounds)
